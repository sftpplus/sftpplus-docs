...
* IBM MQ server support was added as a location that can be used to upload and
  download files. [#7197]
...
* The Web Manager UI for configuring seconds will no longer automatically
  ...
  it was automatically converted to 1 hour.
...
* The Web Manager UI is now functional for administrators that have
  permissions to only manage the configuration,
  without any permissions to server's operations.
  ...
  [manager] [#7243]
* The locations and event handlers `Test` functionality now works for newly
  created components that require a password.
  [manager] [#7230]
...
* The `/runnables/` role permissions were renamed to `/operation/`.
  The configuration is automatically migrated.
  The `/runnables/` permissions were accidentally documented in version 5.1.0.
  [manager] [#7243]
