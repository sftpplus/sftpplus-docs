...
SFTPPlus can operate normally when anti-virus applications are installed to protect the environment on the machine.

Most anti-virus applications have a real-time protection component that will scan files on creation, when accessed, and on execution.
