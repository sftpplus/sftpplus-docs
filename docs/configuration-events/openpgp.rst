...
Below you can find all the configuration options available for the `openpgp` event handler.
...
:Description:
    ...
    It can contain one or multiple public PGP keys.

    The values are the UUIDs of the public PGP keys stored as vault items.
...
:Description:
    ...
    It can contain one or multiple private PGP keys.

    The values are the UUIDs of the private PGP keys stored as vault items.
