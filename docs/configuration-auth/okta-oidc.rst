...
From the `Sign On` tab of your app integration, go to `OpenID Connect ID Token` section and click `Edit`.
Make sure `Groups claim type` is set to `Filter`.
For `Groups claim filter`, type `groups`, then select `Matches regex` and enter `.*` as the value.
This will allow SFTPPlus to receive the Okta groups of the authenticated user.
You can configure a different filter.

Finally, make sure that the users that need to authenticate in SFTPPlus are assigned to this Okta app integration.
...
:From version: 5.26.0
:Description:
    Defines additional certificate authorities trusted for outgoing Okta HTTPS connections.

    The DigiCert certificate authorities distributed with SFTPPlus are always trusted.
