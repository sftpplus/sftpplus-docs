...
Below you can find all the configuration options available for groups.
...
:Values: * `yes`
         * `no`
:Description:
    Whether to allow SSH host-based authentication (hostbased) as specified in RFC 4252 section 9,
    for users that have this group as the primary group.

    The public key is validated using the same authorized keys as for the `publickey` authentication method.

    The client host name and user name of the client host values are only used to validate the signature.
